...
def answered(orch: Orchestrator, digest: str, game_uid: str = "") -> None:
    """Push the digest the opponent would have sent, through the real door.

    The gate consumes what the opponent pushes at *us* rather than reading the
    ``ok`` they answered our push with — ``negotiate`` acknowledges any
    well-formed message, so that ``ok`` is identical whether they compared our
    parameters or never looked at them.
    """
...
class TestConfigAgreement:
    ...
    def test_their_ok_is_not_their_agreement(self) -> None:
        """The whole of P0-1 in one assertion.

        ``negotiate`` returns ``{"ok": True}`` for any well-formed mapping, so
        a peer that never compared our parameters acknowledges us exactly as one
        that agreed would. Reading that ``ok`` as consent is how two peers on
        different physics played a full series and scored zero each.
        """
        config = shipped()
    ...
    def test_silence_after_an_ok_is_a_timeout(self) -> None:
        """An unanswered agreement is a refusal to agree, not a longer wait."""
        orch, _ = orchestrator({"ok": True})
        with pytest.raises(MatchAborted) as excinfo:
...
class TestForgeryBecomesAResult:
    ...
    def test_a_forged_step_aborts_with_forgery(self) -> None:
        """The one unappealable verdict in the system.

        Everywhere else a failure is a technical loss both teams reconcile; a
        commitment that does not open to the move it was revealed as is proof
    ...
    def test_the_arithmetic_travels_with_the_accusation(self) -> None:
        """An accusation this serious has to arrive so the other side can run it."""
        from test_ceremony import honest_match, reveal
