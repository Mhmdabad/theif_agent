...
| 7 | **The commitment formula itself.** The book's `commit()` (p. 37) folds the nonce into the record as a field and leaves `ensure_ascii` at its default; the reference's `CommitReveal.commit_of` appends it after a pipe — `SHA256(canonical_json + "\|" + nonce)` — with `ensure_ascii=False`. They agree on no input at all. | **The reference's form on the wire; either accepted on verify.** `domain/crypto.py` seals the pipe form; `domain/crypto_wire.py` keeps the book's, so `verify` and the Replay App still open a literal-book opponent's records — and our own logs sealed before the flip. `tests/fixtures/commit_vectors.json` carries both digests per record, the wire column computed by the reference repository's own code. | The digest is judged by the *opponent's* auditor, and the reference's recomputes only its own form: book-form records verify **0-of-N** under it (measured against the actual repository), which rule 19 reads as forgery — unappealable, zero for both sides. The asymmetry runs one way, since accepting both costs nothing and sending the book's form risks everything. And p. 37 itself concedes the operational record to the reference — *"the reference code seals a richer record … the core is shown"* — the same settlement entry 4 reached: the opponent's parser owns the wire. |

Entries 1, 2, 4, 5, 6 and 7 are divergences between the rulebook and the
reference code published alongside it, and the default is that the **PDF is
authoritative** — its own header states that the source PDF governs and that
Appendix F is the sole authority for quantitative parameters. The reference
implementation is treated as one more implementation we may have to negotiate
against, not as a standard. The exceptions are where a value is read by the
opponent's parser or by nobody: entry 4's key names and entry 7's digest,
where matching the implementation the cohort actually runs is the entire
